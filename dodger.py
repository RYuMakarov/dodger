...
while True:
    ...
    while True: # Игровой цикл выполняется, пока игра работает.
        ...
        for event in pygame.event.get():
            ...
            if event.type == KEYDOWN:
                if event.key == K_z: # Активация чит-кода обратного направления.
                    reverseCheat = True
                if event.key == K_x: # Активация чит-кода замедления.
                    slowCheat = True
    ...
    drawText('Нажмите клавишу для начала новой игры', font, windowSurface, (WINDOWWIDTH / 3) - 180, (WINDOWHEIGHT / 3) + 50)
    pygame.display.update()
    waitForPlayerToPressKey()
